...
//下面四个时间选择器 改的很简单就是 后面的不能比前面的小就行了
    ...
    $(".bookAdd_btn").click(function () {
        var addIndex=top.layer.open({
            title:"添加图书",
            type:2,
            area:['550px','590px'],
            content:ctx+"/book/addBook"
        });
    });
    ...
    table.on('tool(bookList)',function (obj) {
        ...
        else if(obj.event=='edit')
        {
            var editIndex=top.layer.open({
                ...
                title:"修改会员信息",
                area:['450px','600px'],
                content:ctx+"/book/editBook/"+data.book_id //controller中只是跳转jsp 所以这里不用success判断
            });
        }
    })
});
